  ...
  return user ? (
    <div className="relative h-full overflow-y-scroll bg-gray-50 p-6">
      <div className="max-w-3xl mx-auto">
        ...
        {/* tabs  */}
        <div className="mt-6">
          <div className="bg-white rounded-xl shadow p-1 flex gap-30 max-w-md mx-auto">
            {["posts", "media", "likes"].map((tab) => (
              <button
                onClick={() => setActiveTab(tab)}
                key={tab}
                className={`flex px-4 py-2 text-sm font-medium rounded-lg transition-colors cursor-pointer ${
                  activeTab === tab
                  ...
                  <React.Fragment key={post._id}>
                    {post.image_urls.map((image) => (
                      ...
                      >
                        <img
                          src={image}
                          className="w-61 aspect-video object-cover rounded-lg"
                          alt="user post"
                        />
